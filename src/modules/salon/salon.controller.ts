  ...
  @ApiBody({
    schema: {
      ...
      properties: {
        ...
        'owner[name]':            { type: 'string', description: "Update owner's display name" },
        'owner[phone]':           { type: 'string', description: "Update owner's phone number" },
        'owner[currentPassword]': { type: 'string', description: 'Required when owner[newPassword] is set (SALON_OWNER only)' },
        'owner[newPassword]':     { type: 'string', description: "New password for the owner account (min 8 chars)" },
        logo:  { type: 'string', format: 'binary', description: 'Logo image (JPEG/PNG/WebP/GIF, max 5 MB)' },
        cover: { type: 'string', format: 'binary', description: 'Cover image (JPEG/PNG/WebP/GIF, max 5 MB)' },
      },
    },
